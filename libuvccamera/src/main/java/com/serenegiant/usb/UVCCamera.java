...
//    		mNativePtr = 0;	// nativeDestroyを呼ぶのでここでクリアしちゃダメ
    ...
    public void setPreviewSize(final int width, final int height, final int min_fps, final int max_fps, final int frameFormat, final float bandwidthFactor) {
        if (DEBUG)
            Log.d(TAG, "setPreviewSize() called with: width = [" + width + "], height = [" + height + "], min_fps = [" + min_fps + "], max_fps = [" + max_fps + "], frameFormat = [" + frameFormat + "], bandwidthFactor = [" + bandwidthFactor + "]");
        if ((width == 0) || (height == 0))
        ...
        if (mNativePtr != 0) {
            ...
            if (result != 0) {
                ...
                if (DEBUG)
                    Log.d(TAG, "setPreviewSize() success! called with: width = [" + width + "], height = [" + height + "], min_fps = [" + min_fps + "], max_fps = [" + max_fps + "], frameFormat = [" + frameFormat + "], bandwidthFactor = [" + bandwidthFactor + "]");
            }
