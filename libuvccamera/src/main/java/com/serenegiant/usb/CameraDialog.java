...
//		mUSBMonitor.dumpDevices();
		...
		DeviceListAdapter mDeviceListAdapter = new DeviceListAdapter(getActivity(), mUSBMonitor.getDeviceList(filter.get(0)));
        ...
        public View getView(final int position, View convertView, final ViewGroup parent) {
            ...
            if (convertView instanceof CheckedTextView) {
                ...
                ((CheckedTextView) convertView).setText(
                        String.format("Camera:(%x:%x:%s:%s)", device.getVendorId(), device.getProductId(), device.getDeviceName(),device.getSerialNumber()));
            }
            return convertView;
        }
    }
