...
static int detach_kernel_driver_and_claim(struct libusb_device_handle *handle, int interface) {
    ...
    if (r == 0 || (r != 0 && errno != ENOTTY)) {
        ...
        switch (errno) {
            case EBUSY: RETURN(LIBUSB_ERROR_BUSY, int);
            case EINVAL: RETURN(LIBUSB_ERROR_INVALID_PARAM, int);
            case ENODEV: RETURN(LIBUSB_ERROR_NO_DEVICE, int);
        }
        usbi_err(HANDLE_CTX(handle),
...
static int op_submit_transfer(struct usbi_transfer *itransfer) {
    ...
    switch (transfer->type) {
        case LIBUSB_TRANSFER_TYPE_CONTROL: {
            r = submit_control_transfer(itransfer);
            if(r==-1){
            	LOGE("submit_control_transfer()->error");
            }
            return r;
        }
        case LIBUSB_TRANSFER_TYPE_BULK:
        case LIBUSB_TRANSFER_TYPE_BULK_STREAM: {
            r = submit_bulk_transfer(itransfer);
			if(r==-1){
				LOGE("submit_bulk_transfer()->error");
			}
            return r;
        }
        ...
        case LIBUSB_TRANSFER_TYPE_ISOCHRONOUS: {
            r = submit_iso_transfer(itransfer);
			if(r==-1){
				LOGE("submit_iso_transfer()->error");
			}
            return r;
        }
        default:
...
static int op_cancel_transfer(struct usbi_transfer *itransfer) {
    ...
    if (UNLIKELY(!tpriv->urbs)) RETURN(LIBUSB_ERROR_NOT_FOUND, int);
...
static int handle_iso_completion(struct libusb_device_handle *handle,    // XXX added saki
    ...
    if (UNLIKELY(urb_idx == 0)) {
        usbi_err(TRANSFER_CTX(transfer),
                 "could not locate urb!");    // crash 2014/09/29 SIGSEGV/SEGV_MAPERR
        usbi_mutex_unlock(&itransfer->lock);
        return LIBUSB_ERROR_NOT_FOUND;
    }
...
static int op_handle_events(struct libusb_context *ctx, struct pollfd *fds,
    ...
    for (i = 0; i < nfds && num_ready > 0; i++) {
        ...
        if (pollfd->revents & POLLERR) {
            ...
            usbi_mutex_lock(
                    &ctx->events_lock);        // XXX as a note of usbi_handle_disconnect shows that need event_lock locked
