...
int API_EXPORTED libusb_submit_transfer(struct libusb_transfer *transfer) {
    ...
    usbi_mutex_lock(&itransfer->lock);
    {
        ...
        if (LIKELY(r == LIBUSB_SUCCESS)) {
            ...
            LOGE("libusb_submit_transfer() add_to_flying_list()  fail r=%d ",r);
        }

        if (UNLIKELY(r != LIBUSB_SUCCESS)) {
            LOGE("libusb_submit_transfer()  submit_transfer() fail r=%d,itransfer_type=%d ",r,itransfer->stream_id);
...
int usbi_handle_transfer_completion(struct usbi_transfer *itransfer,
    ...
    if (status == LIBUSB_TRANSFER_COMPLETED
        ...
        if (rqlen !=
            itransfer->transferred) {    // XXX itransfer->transferred is almost always zero on iso transfer mode...
            usbi_dbg("interpreting short transfer as error");
            LOGI("interpreting short transfer as error:rqlen=%d,transferred=%d", rqlen,
...
int API_EXPORTED libusb_wait_for_event(libusb_context *ctx, struct timeval *tv) {
    ...
    r = usbi_cond_timedwait(&ctx->event_waiters_cond,
                            ...
                            &timeout);    // XXX crash 2014/10/02 SIGABRT/SI_TKILL
    return (r == ETIMEDOUT);
}
...
static int handle_events(struct libusb_context *ctx, struct timeval *tv) {
    ...
    {
        list_for_each_entry(ipollfd, &ctx->pollfds, list, struct usbi_pollfd) nfds++;
...
void usbi_remove_pollfd(struct libusb_context *ctx, int fd) {
    ...
    usbi_mutex_lock(&ctx->pollfds_lock);
    {
        list_for_each_entry(ipollfd, &ctx->pollfds, list, struct usbi_pollfd) if (
                    ipollfd->pollfd.fd == fd) {
                found = 1;
                break;
            }
...
const struct libusb_pollfd **LIBUSB_CALL libusb_get_pollfds(
    ...
    usbi_mutex_lock(&ctx->pollfds_lock);
    {
        list_for_each_entry(ipollfd, &ctx->pollfds, list, struct usbi_pollfd) cnt++;
        ...
        list_for_each_entry(ipollfd, &ctx->pollfds, list,
                            struct usbi_pollfd) ret[i++] = (struct libusb_pollfd *) ipollfd;
        ret[cnt] = NULL;
    }
    out:
    usbi_mutex_unlock(&ctx->pollfds_lock);
...
void usbi_handle_disconnect(struct libusb_device_handle *handle) {
    ...
    while (1) {
        ...
        list_for_each_entry(cur, &HANDLE_CTX(handle)->flying_transfers, list,
                            struct usbi_transfer) if (
                    USBI_TRANSFER_TO_LIBUSB_TRANSFER(cur)->dev_handle == handle) {
                to_cancel = cur;
                break;
            }
